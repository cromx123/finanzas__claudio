...
def test_alerts_scoped_to_owner(client, monkeypatch):
    ...
    # Deleting someone else's alert is a silent no-op, not a leak/404 that
    # would confirm the id exists — mirrors delete_transaction/delete_portfolio.
    client.delete(f"/v1/alerts/{alert_id}", headers=_auth(token_b))
